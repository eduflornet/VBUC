...
namespace UpgradeHelpers
{
    ...
    partial class DataGridViewFlex
    {
        ...
        {
            //Next two lines commented out to keep the wanted behavior defined in design properties.
        ...
        /// <summary>
        /// Gets/Sets column widths, alignments, and fixed row and column text.
        /// </summary>
        public string FormatString
        {
            ...
            set
            {
                ...
                UpdateColumnsFormat(this._formatString);
            }
        }

        /// <summary>
        /// Returns/Sets an object in a control.
        /// </summary>
        public object Object { get; set; }

        /// <summary>
        ///     Gets or sets a value indicating whether the width of the row headers is adjustable
        ///     and whether it can be adjusted by the user or is automatically adjusted to fit
        ///     the contents of the headers.
        ///
        ///  Note: This property overrides in this implementation to redefine a new default value that
        ///       matches to the original behavior of the replaced Activex control (see Reset() method).
        /// </summary>
        ...
        /// <summary>
        ///     Gets or sets a value indicating whether the user can edit the cells of the System.Windows.Forms.DataGridView
        ///     control.
        ///
        ///  Note: This property overrides in this implementation to redefine a new default value that
        ///       matches to the original behavior of the replaced Activex control (see Reset() method).
        ...
        /// <summary>
        ///     Gets or sets a value indicating whether the height of the column headers is adjustable
        ///     and whether it can be adjusted by the user or is automatically adjusted to fit
        ///     the contents of the headers.
        ///
        /// Note: This property overrides in this implementation to redefine a new default value that
        ///       matches to the original behavior of the replaced Activex control (see Reset() method).
        /// </summary>
        [DefaultValue(System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.DisableResizing)]
        ...
        public void Set_ColPosition(int index, int newIndex)
        {
            DataGridViewColumnCollection columnsCollection = this.Columns;
            DataGridViewColumn column = columnsCollection[index - 1].Clone() as DataGridViewColumn;
            List<DataGridViewCell> cells = new List<DataGridViewCell>();

            for (int i = 0; i < this.RowCount; i++)
            {
                cells.Add(this.Rows[i].Cells[index - 1]);
            }

            columnsCollection.RemoveAt(index - 1);
            columnsCollection.Insert(newIndex - 1, column);

            for (int i = 0; i < this.RowCount; i++)
            {
                this.Rows[i].Cells[newIndex - 1] = cells[i];
            }
        }
        ...
        public void Clear()
        {
            DataGridViewCellStyle fixedStyle = GetCellStyleFixed();
            DataGridViewCellStyle nonFixedStyle = GetCellStyleNonFixed();

            //avoid recalculation of properties
            int realRows = RowsCount;
            int realCols = ColumnsCount;
            int fixedRows = FixedRows;
            int fixedCols = FixedColumns;

            // loop to access all cells, including fixed
        ...
        public void AddItem(string vsItem)
        {
			...
			bool tempSuppressSelectionChanged = _suppressSelectionChanged;
			_suppressSelectionChanged = true;
			Rows.Add(row);
			_suppressSelectionChanged = tempSuppressSelectionChanged;
        ...
        /// If value is set to 0 (zero) the column must be hidden
        /// </summary>
        ...
        {
            pos = Math.Min(Columns.Count - 1, pos);
            if (pos == 0 && RowHeadersVisible)
            {
                if (value == 0)
                {
            ...
            else
            {
                if (RowHeadersVisible)
                {
                    ...
                    this.Columns[pos - 1].Visible = value != 0;
                }
                else
                {
                    this.Columns[pos].Width = (int)value;
                    this.Columns[pos].Visible = value != 0;
                }
            }
        }
        ...
        public void SetRowHeight(int pos, double value)
        {
            ...
            else
            {
                if (ColumnHeadersVisible)
                {
                    ...
                    this.Rows[pos - 1].Visible = value != 0;
                }
                else
                {
                    this.Rows[pos].Height = (int)value;
                    this.Rows[pos].Visible = value != 0;
                }
            }
        }
        ...
        public DataGridViewCell GetCell(int rowindex, int colIndex)
        {
            ...
            {
                int realRow = Math.Min(Rows.Count - 1, rowindex);
                int realCol = Math.Min(Columns.Count - 1, colIndex);
                return base[realCol, realRow];
            }
        }
        ...
        private void UpdateColumnsFormat(string formatString)
        {
            if (!String.IsNullOrEmpty(formatString))
            {
                int colFormatIndex = 0;
                string[] colFormats = formatString.Split(new char[] { '|' });

                // update row header column
                for (; colFormatIndex < Math.Min(1, this.FixedColumns); colFormatIndex++)
                {
                    UpdateHeaderCellInfo(this.TopLeftHeaderCell, colFormats[colFormatIndex]);
                    this.RowHeadersWidth = (int)(((string)this.TopLeftHeaderCell.Value).Length * this.Font.SizeInPoints);
                }

                // update current columns
                for (; colFormatIndex < Math.Min(this.Columns.Count + 1, colFormats.Length); colFormatIndex++)
                {
                    UpdateColumnInfo(this.Columns[colFormatIndex - 1], colFormats[colFormatIndex]);
                }

                // create additional columns (if necessary)
                for (; colFormatIndex < colFormats.Length; colFormatIndex++)
                {
                    DataGridViewColumn col = new DataGridViewColumn();

                    // set column info
                    col.CellTemplate = this.Columns[this.ColumnsCount - 2].CellTemplate;
                    UpdateColumnInfo(col, colFormats[colFormatIndex]);
        ...
        private void UpdateHeaderCellInfo(DataGridViewHeaderCell cell, string columnFormatString)
        {
            int startIndexName = 1;

            // update header alignment
            switch (columnFormatString[0])
            {
                case '<':
            ...
            // update header text
            cell.Value = columnFormatString.Substring(startIndexName);
        }

        private void UpdateColumnInfo(DataGridViewColumn column, string columnFormatString)
        {
            int startIndexName = 1;

            // update header cell
            UpdateHeaderCellInfo(column.HeaderCell, columnFormatString);
            ...
            // update column name
            column.Name = columnFormatString.Substring(startIndexName);

            // update column width
            column.Width = (int)(column.HeaderText.Length * this.Font.SizeInPoints);
        }
