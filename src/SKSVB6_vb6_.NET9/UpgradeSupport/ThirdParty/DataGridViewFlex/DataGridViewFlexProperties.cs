                ...
                else
                {
                    ...
                    if (value > 1)
                    {
                        ...
                        if (Columns.Count < realvalue)
                        {
                            ...
                            //Now we have to apply correct style to rows
                            DataGridViewCellStyle cellstyleFixed = GetCellStyleFixed();
                            DataGridViewCellStyle cellstyleNormal = GetCellStyleNonFixed();

                            // if ColHeaderVisible then Rows does not count the first row (header)
                            int fixedRows = ColumnHeadersVisible ? FixedRows - 1 : FixedRows;
				...
				try
				{
					...
					else
					{
						...
						if (Rows.Count < realvalue)
						{
							...
							//Now we have to apply correct style to columns
							DataGridViewCellStyle cellstyleFixed = GetCellStyleFixed();
							DataGridViewCellStyle cellstyleNonFixed = GetCellStyleNonFixed();

							// if RowHeadersVisible then Columns does not count the first col (header)
							int fixedCols = RowHeadersVisible ? FixedColumns - 1 : FixedColumns;
							foreach (DataGridViewColumn col in Columns)
							{
								if (col.Index < fixedCols)
								{
									col.DefaultCellStyle = cellstyleFixed;
								}
								else
								{
									col.DefaultCellStyle = cellstyleNonFixed;
								}
							}
						}
        ...
        public int GridLineWidth
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        _gridLineWidth = Math.Max(0, value - 1);
            ...
            set
			{
				...
				try
				{
					...
					else
					{
						...
						// throw vb6 exception.
						if (value >= _rows && DesignMode)
						{
							throw new NotSupportedException("FixedRows must be at least one less than Rows value");
						}
						...
						else
						{
							...
							{
								// If there is no header but (new) value > 0
								if (Rows.Count > 0)
								{
									// We copy everything from the first row to the actual header and remove first row
							...
							foreach (DataGridViewRow row in Rows)
							{
								Boolean rowFrozen = row.Index < realvalue;
								row.Frozen = rowFrozen;

								// Freeze/Unfreeze cells depending of fixed columns
								foreach (DataGridViewCell cell in row.Cells)
								{
									cell.Style = rowFrozen || cell.ColumnIndex < fixedCols ? cellStyleFixed : cellStyleNormal;
								}
							}
						}
						Refresh();
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (value == 0)
                    {
                        ...
                        newCustomColumn.Name = "Column" + Columns.Count;
                        newCustomColumn.DividerWidth = _gridLineWidth;
                        newCustomColumn.Width = RowHeadersWidth;
                        Columns.Insert(Columns.Count, newCustomColumn);

                        // Updating the new first column, setting values and frozing cells (if applies)
                        DataGridViewCellStyle cellStyleFixed = GetCellStyleFixed();
