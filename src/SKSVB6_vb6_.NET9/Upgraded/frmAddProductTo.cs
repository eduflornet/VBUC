	...
	: System.Windows.Forms.Form
{
		...
		if (m_vb6FormDefInstance is null)
		{
			...
			else
			{
				try
				{
					...
					if (System.Reflection.Assembly.GetExecutingAssembly().EntryPoint is not null && System.Reflection.Assembly.GetExecutingAssembly().EntryPoint.DeclaringType == this.GetType())
					{
						m_vb6FormDefInstance = this;
					}
				}
	...
	public void LoadData()
	{
		...
		productsStored = new (System.StringComparer.OrdinalIgnoreCase);
		productsToDelete = new (System.StringComparer.OrdinalIgnoreCase);
		productsToAdd = new (System.StringComparer.OrdinalIgnoreCase);
		LoadProductsById();
	}
		...
		if (modConnection.rs.RecordCount > 0)
		{
			while (!modConnection.rs.EOF)
			{
				...
				for (modMain.i = 1; modMain.i <= 2; modMain.i++)
				{
					...
					if (modConnection.rs.GetField(modMain.i) is not null)
					{
	...
	private void AddProductToSet()
	{
		...
		ListViewItem x = null;
		//UPGRADE_WARNING: (2080) IsEmpty was upgraded to a comparison and has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis/warnings#id-2080
		if (lvProducts.FocusedItem is not null)
		{
